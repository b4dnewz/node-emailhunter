...
import LeadsList from './leads_lists.js';
...
export default class EmailHunter {
  ...
  run({ action = 'GET', type, id = '', options = {}, callback = () => {} }) {
    ...
    // Basic url
    let url = `${API_ENDPOINT}/${type}`;

    // Optionally add endpoint id
    if (id) {
      url += `/${id}`;
    }
  ...
  /**
   * The object which contains all the leads_lists methods
   * @return {LeadsList} The leads_lists API methods
   */
  get leadsList() {
